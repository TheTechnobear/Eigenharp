...
struct osc_wire_t:
    ...
    pic::element_t<>
{
    osc_wire_t(osc_pad_t::impl_t *p, const piw::event_data_source_t &);
...
{
	light_wire_t(osc_pad_t::impl_t *impl);
	~light_wire_t();
...
osc_pad_t::impl_t::~impl_t()
{
	// pic::logmsg() << "osc_pad_t::impl_t::~impl_t";
...
void osc_pad_t::impl_t::set_window(unsigned top, unsigned height, unsigned left, unsigned width)
{
//	pic::logmsg() << "osc_pad_t::impl_t::set_window "<< top << "," << height << "," << left << "," << width;
	piw::tsd_fastcall(__set_window, light_wire_.ptr(), new window(client_,view(), top, height, left, width));
}



void osc_pad_t::impl_t::refresh()
{
//	pic::logmsg() << "osc_pad_t::impl_t::refresh";
	piw::tsd_fastcall(__refresh, light_wire_.ptr(), new window(client_,view(), 0, 0, 0, 0));
}

void osc_pad_t::impl_t::play()
{
//	pic::logmsg() << "osc_pad_t::impl_t::play";
	client_->send(new live_play_message_t());
}

void osc_pad_t::impl_t::stop()
{
//	pic::logmsg() << "osc_pad_t::impl_t::refresh";
	client_->send(new live_stop_message_t());
}

void osc_pad_t::impl_t::play_scene(int scene)
{
//	pic::logmsg() << "osc_pad_t::impl_t::play_scene" << scene;
	client_->send(new live_play_scene_message_t(scene));
}

void osc_pad_t::impl_t::shutdown()
{
	// pic::logmsg() << "osc_pad_t::impl_t::shutdown";
	client_->osc_shutdown();
	server_->osc_shutdown();
}
...
void osc_pad_t::impl_t::add_ticker(osc_wire_t *w)
{
	if(!tickers_.head())
	{
		tick_suppress(false);
...
void osc_pad_t::impl_t::del_ticker(osc_wire_t *w)
{
	tickers_.remove(w);
...
void osc_pad_t::impl_t::invalidate()
{
	tick_disable();
...
piw::wire_t *osc_pad_t::impl_t::root_wire(const piw::event_data_source_t &es)
{
   pic::lckmap_t<piw::data_t,osc_wire_t *>::lcktype::iterator ci;
...
void osc_pad_t::impl_t::root_closed() { invalidate(); }

void osc_pad_t::impl_t::root_opened() { root_clock(); root_latency(); }

void osc_pad_t::impl_t::root_clock()
{
	if(up_)
	{
		remove_upstream(up_);
...
osc_wire_t::osc_wire_t(osc_pad_t::impl_t  *p, const piw::event_data_source_t &es):
...
//    	pic::logmsg() << "osc_wire_t::process " << s  << "track" << track << "clip:" << clip << "hard" << hardness;
...
//////////// osc_pad_t
osc_pad_t::osc_pad_t(piw::clockdomain_ctl_t *d, const piw::cookie_t &output,const std::string &sendhost, const std::string &sendport, const std::string &recvport)
...
piw::cookie_t osc_pad_t::cookie()
{
    return piw::cookie_t(impl_);
}


void osc_pad_t::set_window(unsigned top, unsigned height, unsigned left, unsigned width)
{
//	pic::logmsg() << "osc_pad_t::set_window "<< top << "," << height << "," << left << "," << width;
...
void osc_pad_t::refresh()
{
//	pic::logmsg() << "osc_pad_t::set_window ";
	impl_->refresh();
}

void osc_pad_t::play()
{
//	pic::logmsg() << "osc_pad_t::play ";
	impl_->play();
}

void osc_pad_t::stop()
{
//	pic::logmsg() << "osc_pad_t::stop ";
	impl_->stop();
}

void osc_pad_t::play_scene(int scene)
{
//	pic::logmsg() << "osc_pad_t::play_scene " << scene;
	if (scene > 0)
	{
